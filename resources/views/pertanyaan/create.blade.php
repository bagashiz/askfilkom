        ...
        <form action="/pertanyaan" method="POST">
            ...
            <div class="m-6">
                ...
                <input type="judul" id="judul" name="judul"
                    ...
                    placeholder="Tanya apa pun seputar FILKOM!" required="" value="{{ old('judul') }}">
                @error('judul')
            ...
            <div class="m-6" x-data="{ showTopik: false }">
                ...
                <div id="topik-container" x-show="showTopik" class="my-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                    @foreach ($topik as $item)
                        <div class="flex items-center">
                            <input type="checkbox" id="topik{{ $item->id_topik }}" name="topik[]"
                                value="{{ $item->id_topik }}"
                                class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                            <label for="topik{{ $item->id_topik }}"
            ...
            <div class="m-6">
                ...
                <textarea id="deskripsi" name="deskripsi"
                    ...
                    placeholder="Jelasin pertanyaan kamu di sini!" required>{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    @include('partials._error-message', ['errorMessage' => $message])
                @enderror
            </div>
